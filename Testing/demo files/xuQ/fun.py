...
#sys.path.append("/usr/local/share/sumo-0.27.0/tools")
sys.path.append("/usr/local/share/sumo-0.27.0/tools")
import traci
import random
import pandas as pd
...
def learnDiscretization(daysToObserve):
    ...
    for day in range(daysToObserve):
        ...
        sumoProcess = subprocess.Popen(['/usr/local/src/sumo-0.27.0/src/sumo', "-c", projectName, \
...
#                 print('si esta')
            ...
            if (True in change):
                ...
                for agt in agentChanged:
                    ...
                    for n in var.agents[agt].neighbors:
                        for edge in var.agents[n].listEdges:
                            stateDataEntry.append(var.agents[n].queueTracker[edge])
                        for edge in var.agents[n].listEdges:
                            stateDataEntry.append(var.agents[n].waitingTracker[edge])
                    if len(stateData[agt][hod]) == 0:
                        stateData[agt][hod] = np.array([stateDataEntry])
                    else:
                        stateData[agt][hod] = np.vstack([stateData[agt][hod], stateDataEntry])
    ...
    for agt in range(var.numAgents):
        fileOut = open("recoveryStates_"+str(agt)+".csv","w")
        for h in range(var.hoursInDay):
            var.agents[agt].numClustersTracker[h] = {}
            var.agents[agt].dictClusterObjects[h] = {}
            
            if(len(stateData[agt][h]) != 0):
                var.agents[agt].numClustersTracker[h] = int(sum(np.std(stateData[agt][h], axis = 0)))
                if(var.agents[agt].numClustersTracker[h] > len(stateData[agt][h])):
                    var.agents[agt].numClustersTracker[h] = len(stateData[agt][h])
                print('------- Number of clusters -------')
                print ('agent = ' + str(agt) )
                print 'h = ', h
                print 'numClustersTracker[agt][h][act] = ', var.agents[agt].numClustersTracker[h]
                    
                var.agents[agt].dictClusterObjects[h] = KMeans(n_clusters=var.agents[agt].numClustersTracker[h])
                var.agents[agt].dictClusterObjects[h].fit(stateData[agt][h])
                coord = var.agents[agt].dictClusterObjects[h].cluster_centers_
                for k in range(0,var.agents[agt].numClustersTracker[h]):
                    fileOut.write(str(h))
                    map(lambda y: fileOut.write("  " + str(y)), coord[k])
                    fileOut.write("\n")
            else:
                print('\n ERROR: Missing data for h:'+str(h)+ 'for agent '+str(agt))
        fileOut.close() 
    ...
    for sl in var.SLs:
        totalClusters = 0;
        for h in range(var.hoursInDay): 
            totalClusters += var.agents[sl].numClustersTracker[h]
    ...
    for sl in var.SLs:
        ...
        for h in range(var.hoursInDay): 
            ...
            for c in range(var.agents[sl].numClustersTracker[h]):
                var.agents[sl].mapDiscreteStates[h][c] = stateCounter
                stateCounter += 1        
