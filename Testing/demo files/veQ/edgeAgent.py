...
class edgeAgent():
    def __init__(self, ag_i, ag_j):
        ...
        self.edge_id = var.Edges.index(self.vertexAgt)
    ...
    def updateState(self, currHod):
        ...
        # if(self.currStateID >= np.shape(self.QValues)[0]):
        #     self.currStateID = np.shape(self.QValues)[0]-1;
    ...
    def loadKnowledge(self, day):
        self.cluster_centers = pd.read_csv('./states/recoveryStates_edge' + str(self.edge_id) +'.csv',
                                sep=' ', skipinitialspace=True, header=None)
        self.cluster_centers = self.cluster_centers.values
        self.QValues = np.loadtxt('./train/QValues_edge' + str(self.SL_i) + '_' +  str(self.SL_j) + '_day' + str(day) +'.txt' )    
