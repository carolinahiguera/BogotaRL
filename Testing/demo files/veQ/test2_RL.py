...
def loadData():
    df=pd.DataFrame.from_csv('./data_org/dfQueue_train_int0_day0.csv')
    ...
    for v in var.Vertex:
        ...
        for day in range(var.totalDaysTest-1):
            df=pd.DataFrame.from_csv(f'./data_org/dfQueue_train_int{v}_day{day}.csv')
            dataQ = df.values
            df=pd.DataFrame.from_csv(f'./data_org/dfWaiting_train_int{v}_day{day}.csv')
            dataW = df.values
            df=pd.DataFrame.from_csv(f'./data_org/dfSpeed_test_int{v}_day{day}.csv')
            dataS = df.values
