...
def test(dayLoad):
    ...
    for day in range(var.totalDaysTest):
        ...
        sumoProcess = subprocess.Popen(['/home/carolina/Programas/sumo-0.27.1/bin/sumo-gui', "-c", projectName, \
        ...
        while currSod < var.secondsInDay: 
            ...
            if(currSod%(var.minTimeInYellow + var.minTimeInGreen) == 0):
                ...
                for sl in var.SLs:
                    ...
                    df = pd.DataFrame([aux])
                    dfMeanSpeed[sl] = dfMeanSpeed[sl].append(df, ignore_index=True)
