...
class agent():
    ...
    def followPolicy(self, currHod):
        for nb in self.neighbors:
            ...
            for edge in var.agents[nb].listEdges:
                state.append(var.agents[nb].waitingTracker[edge])
            state = np.array(state)
            res = np.linalg.norm(state.T-self.cluster_centers[nb], axis=1, ord=2)
            self.currJointStateID[nb] = np.argmin(res)  
            #get joint action
            ki_kj = self.getJointAction(nb)            
        #get self best response
        ai = -1
    ...
    def loadKnowledge(self, day):
        for nb in self.neighbors:
            ...
            aux = pd.read_csv('./states/recoveryStates_' + str(self.SL) + '_' + str(nb) +'.csv',
                                sep=' ', skipinitialspace=True, header=None)
            self.cluster_centers[nb] = aux.values
