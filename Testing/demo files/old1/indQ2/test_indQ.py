...
def getTravelTimes():
    ...
    for r in range(len(var.routes)):
        for i in range(len(var.routes[r]['route_agents'])):
            ...
            size = np.array(dfQueueTracker[agent]).shape
            if size[0]>ORDER:
                num_veh = np.mean(np.array(dfQueueTracker[agent])[size[0]-ORDER:size[0],edge])
                waitTime = np.mean(np.array(dfWaitingTracker[agent])[size[0]-ORDER:size[0],edge])/num_veh if num_veh>0 else 0
                speed = np.mean(np.array(dfSpeedTracker[agent])[size[0]-ORDER:size[0],edge])
            else:
    ...
    #print(travel_times)
    # agent=4; edge=1;
    # print(f'Colas:{np.array(dfQueueTracker[agent])[:,edge]}')
    # print(f'Tiempo:{np.array(dfWaitingTracker[agent])[:,edge]}')
    # print(f'Vel:{np.array(dfSpeedTracker[agent])[:,edge]}')
    # print()
    return travel_times
...
for sl in var.SLs:
    var.agents[sl].loadKnowledge(99)
    ...
    while currSod < var.secondsInDay: 
        ...
        if(currSod%(var.minTimeInYellow + var.minTimeInGreen) == 0):
            ...
            for sl in var.SLs:
                ...
                aux = [currSod]
                for edge in var.agents[sl].listEdges:
                    var.agents[sl].speedTracker[edge] = traci.edge.getLastStepMeanSpeed(str(edge))                      
