...
def getCurrentAction():
    for tls in var.agent_TLS.keys():
        phase = var.agent_TLS[tls].phases.index(traci.trafficlight.getPhase(str(tls)))
        if phase==0 or phase==1:
            var.agent_TLS[tls].currAction = phase
        else:
            var.agent_TLS[tls].finishPhase[1] = False

def getObservationNow():
    for tls in var.agent_TLS.keys():        
